...
- **ทุกครั้ง** ที่มีการแก้ไขโค้ดใน `ea-server` (รวมถึง EA `EATradingClient.mq5`) จะต้อง:
  1. **เพิ่มเวอร์ชัน (Bump version)** เสมอ ตามกฎข้อ 3 ด้านล่าง
  2. **Commit และ Push ขึ้น GitHub ทันที** (`git commit -am "..." && git push origin main`)
- เพื่อให้เครื่องเซิร์ฟเวอร์หลัก (VPS) สามารถดึงโค้ดล่าสุดจาก GitHub ไปใช้งานต่อได้ทันที
