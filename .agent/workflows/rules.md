...
- **ทุกครั้ง** ที่มีการแก้ไขโค้ดใน `ea-server` (รวมถึง `src/main.rs`, `src/db.rs`, EA `EATradingClient.mq5`, หรือไฟล์ใดๆ ภายใต้ `ea-server/`) จะต้อง:
  1. **เพิ่มเวอร์ชัน (Bump version) ใน `ea-server/Cargo.toml`** เสมอ (ห้ามลืมเด็ดขาด!)
  2. **Commit และ Push ขึ้น GitHub ทันที** (`git commit -am "..." && git push origin main`)
- **เหตุผลสำคัญสูงสุด (ทำไมต้องเปลี่ยนเวอร์ชันทุกครั้ง):** เซิร์ฟเวอร์ที่กำลังรันอยู่บน VPS (หรือเครื่องลูกค้า) มีระบบคอยเช็ค Auto-Update ทุกๆ 2 ชั่วโมง ถ้าเราไม่แก้เวอร์ชัน (เลขเวอร์ชันใน `Cargo.toml` ยังเป็นตัวเดิม) **เครื่องที่ใช้งานอยู่มันจะไม่ยอมโหลดโปรแกรมเวอร์ชันใหม่ไปติดตั้งโดยอัตโนมัติ** ทำให้บั๊กยังคงอยู่เหมือนเดิม!
- **ขั้นตอนหลังแก้โค้ด ea-server ทุกครั้ง:**
  1. Bump version ใน `ea-server/Cargo.toml` (เช่น `5.9.13` → `5.9.14`)
  2. `git commit -am "fix/feat: ..." && git push origin main`
  3. **สร้าง Git Tag ให้ตรงกับเวอร์ชัน** แล้ว push: `git tag v5.9.14 && git push origin v5.9.14`
  4. รอ GitHub Actions คอมไพล์ Release ใหม่ (~3-5 นาที)
  5. กดปุ่ม "Update EA-24 Server" บน VPS (หรือรอ auto-update)

### 2. ฝั่ง Client (ea-client / Rust Desktop)
- **ในการทำงานปกติ ให้รันแค่ Client ที่เป็น Rust Desktop (Tauri) เท่านั้น**
- **พัฒนา/ทดสอบ:** `cd ea-client && npm run tauri dev`
- **Build Production:** `cd ea-client && npm run tauri build` (build ในเครื่อง ไม่ใช้ GitHub Actions)
- **ห้าม Build Desktop Client ผ่าน GitHub Actions** — ใช้การ build ในเครื่องแทนเสมอ
...
#### ⚠️ หมายเหตุ และกฎเหล็กการ Release:
...
- 🚨 **สำคัญที่สุด (CRITICAL RULE):** ทุกครั้งที่เพิ่มเวอร์ชันของ `ea-server` ขึ้นสู่ GitHub **ต้องทำการผูก Tag ประกาศ Release ใหม่เสมอ** (ตัวอย่าง: `git tag v0.5.4 && git push origin v0.5.4`) เพราะถ้าไม่มี Tag ระบบ CI/CD จะไม่สร้างไฟล์ให้ดาวน์โหลด และ VPS จะอัปเดตไม่ได้! ห้ามลืมขั้นตอนนี้เด็ดขาด!

### 4. กฎ UI/UX — ข้อห้ามเด็ดขาด!
- **ห้ามใช้ Emoji มาทำเป็น Icon ใน UI เด็ดขาด!** ต้องใช้ Icon จาก Library เท่านั้น (เช่น `react-icons/lu`, `lucide-react`)
- Emoji ที่แสดงใน Log จาก Server ต้องถูก strip ออกก่อนแสดงผลเสมอ
- ใช้ `stripEmojis()` helper function สำหรับข้อความที่มาจาก Server
กฎข้อที่ 10: ไฟล์ Implementation Plan ทุกไฟล์ต้องถูกเขียนเป็น "ภาษาไทย" เท่านั้น เพื่อความสะดวกในการอ่านและตรวจสอบ
