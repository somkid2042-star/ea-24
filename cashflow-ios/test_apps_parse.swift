...
if let d = try? JSONSerialization.jsonObject(with: dData) as? [String: Any] {
    ...
    if let apps = d["apps"] as? [[String: Any]] {
        ...
        if apps.count > 0 {
            print("FIRST APP keys:", apps[0].keys)
        }
    } else {
