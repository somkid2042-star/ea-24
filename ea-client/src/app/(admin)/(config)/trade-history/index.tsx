    ...
    <main className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-default-100 dark:bg-default-200/10 px-2.5 py-0.5 text-[10px] font-medium text-default-600">{totalDeals} deals</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        <div className="card !p-5">
          ...
          <p className={`text-2xl font-bold ${netProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{netProfit >= 0 ? '+' : ''}${netProfit.toFixed(2)}</p>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
              ...
              {symbolStats.map(([sym, stat]) => (
                <div key={sym} className="flex items-center gap-3">
                  <span className="min-w-[80px] text-sm font-bold text-primary">{sym}</span>
                    <div className="flex-1 h-2 rounded-full bg-default-100 dark:bg-default-200/10 overflow-hidden">
                  ...
                  <span className={`min-w-[70px] text-right text-xs font-bold ${stat.profit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{stat.profit >= 0 ? '+' : ''}${stat.profit.toFixed(2)}</span>
                </div>
              ))}
            </div>
        ...
        <div className="card !p-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between"><span className="text-xs text-default-500">Gross Profit</span><span className={`text-sm font-bold ${totalProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{totalProfit >= 0 ? '+' : ''}${totalProfit.toFixed(2)}</span></div>
            <div className="flex items-center justify-between"><span className="text-xs text-default-500">Total Swap</span><span className={`text-sm font-bold ${totalSwap >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{totalSwap >= 0 ? '+' : ''}${totalSwap.toFixed(2)}</span></div>
            <div className="flex items-center justify-between"><span className="text-xs text-default-500">Total Commission</span><span className={`text-sm font-bold ${totalComm >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{totalComm >= 0 ? '+' : ''}${totalComm.toFixed(2)}</span></div>
            <hr className="border-default-200/60 dark:border-default-300/10" />
            <div className="flex items-center justify-between"><span className="text-sm font-semibold text-default-900">Net Profit</span><span className={`text-lg font-bold ${netProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{netProfit >= 0 ? '+' : ''}${netProfit.toFixed(2)}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-default-200/60 dark:border-default-300/10 flex items-center justify-between">
            ...
            <tbody className="divide-y divide-default-200/60 dark:divide-default-300/10">
              {deals.length > 0 ? [...deals].reverse().map(d => {
                ...
                <tr key={d.ticket} className="hover:bg-default-50/50 dark:hover:bg-default-100/30 transition">
                  ...
                  <td className={`px-4 py-3 tabular-nums font-bold ${d.profit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{d.profit >= 0 ? '+' : ''}${d.profit.toFixed(2)}</td>
                  ...
                  <td className={`px-4 py-3 tabular-nums font-bold ${net >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{net >= 0 ? '+' : ''}${net.toFixed(2)}</td>
